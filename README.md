# Indtroduction

I hope to make documentation much faster using this extension.


# Content

## Scripts

### report.js
This file contains the code for the structure of generating the report. Used to help manage the data from the webpage.

### popup.js
This file controls the actions on the html popup for the extension.

### parser.js
This file parses the content of the webpage and generates the report for it

## HTML

### hello.html
Contains the html popup for the extension. Has a button used to generate the report.